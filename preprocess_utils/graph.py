...
def concepts_to_adj_matrices_2hop_all_pair__use_LM__Part1(data):
    ...
    for qid1 in q_nodes:
        for qid2 in q_nodes:
            if qid1!=qid2 and qid1 in cpnet_simple.nodes and qid2 in cpnet_simple.nodes:
                extra_nodes |= set(cpnet_simple[qid1]) & set(cpnet_simple[qid2])
    #aa_relation
    for aid1 in a_nodes:
        for aid2 in a_nodes:
            if aid1!=aid2 and aid1 in cpnet_simple.nodes and aid2 in cpnet_simple.nodes:
                extra_nodes |= set(cpnet_simple[aid1]) & set(cpnet_simple[aid2])
    #qa_relation
    for qid in q_nodes:
        for aid in a_nodes:
            if qid!=aid and qid in cpnet_simple.nodes and aid in cpnet_simple.nodes:
...
def generate_adj_data_from_grounded_concepts__use_LM(grounded_path, cpnet_graph_path, cpnet_vocab_path, output_path, num_processes):
    ...
    qa_data = []   
    statement_path = grounded_path.replace('re_grounded', 'statement')
    with open(grounded_path, 'r', encoding='utf-8') as fin_ground, open(statement_path, 'r', encoding='utf-8') as fin_state:
        ...
        for j, line in enumerate(lines_ground):
            ...
            a_ids = set(concept2id[c] for c in dic['ac'])  
            q_ids = q_ids - a_ids
            statement_obj = json.loads(lines_state[j])
            QAcontext = "{}".format(statement_obj['question']['stem'])
