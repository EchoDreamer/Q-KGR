...
class similarity_compute(nn.Module):
    def __init__(self,question_rel_similarity,edge_class,concept_dim=200):
        super().__init__()
        self.question_rel_similarity=question_rel_similarity
        self.edge_class=edge_class
        if self.question_rel_similarity=='BiLinearSimilarity':
                self.sim_BiLinear=nn.Bilinear(in1_features=concept_dim,in2_features=concept_dim*2,out_features=2,bias=True)
        elif self.question_rel_similarity=='ProjectedDotProductSimilarity':
            pass
        elif self.question_rel_similarity=='DotProductSimilarity':
            pass
        elif self.question_rel_similarity=='MultiHeadedSimilarity':
            pass
        else:
            raise ValueError('question_rel_similarity must be one of BiLinearSimilarity,ProjectedDotProductSimilarity,DotProductSimilarity,MultiHeadedSimilarity')
        

    def forward(self,input1,input2) ->torch.tensor:
        if self.question_rel_similarity=='BiLinearSimilarity':
            if self.edge_class=='hard':
                return F.gumbel_softmax(self.sim_BiLinear(input1,input2),hard=True,dim=-1)
            elif self.edge_class=='soft':
                return F.gumbel_softmax(self.sim_BiLinear(input1,input2),hard=False,dim=-1)
            else:
                raise ValueError('edge_class must be one of hard, soft')
        elif self.question_rel_similarity=='ProjectedDotProduc does not match weight size: tSimilarity':
            pass
        elif self.question_rel_similarity=='DotProductSimilarity':
            pass
        elif self.question_rel_similarity=='MultiHeadedSimilarity':
