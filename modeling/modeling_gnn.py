...
class GATConvE(MessagePassing):
    ...
    def forward(self, x, edge_index, edge_type, node_type, node_feature_extra,edge_score=None, return_attention_weights=False):
        ...
        if edge_score is not None:
            edge_score = torch.cat([edge_score[:,0], torch.ones(x.size(0)).to(edge_score.device)], dim=0) #[E+N, 1] 
        x = torch.cat([x, node_feature_extra], dim=1)
        x = (x, x)
        aggr_out = self.propagate(edge_index, x=x, edge_attr=edge_embeddings,edge_score=edge_score) #[N, emb_dim]
    ...
    def message(self, edge_index, x_i, x_j, edge_attr,edge_score=None): #i: tgt, j:src
        ...
        if edge_score is not None:
            msg=msg*edge_score.unsqueeze(-1).unsqueeze(-1)
